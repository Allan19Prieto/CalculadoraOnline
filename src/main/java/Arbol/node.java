...
class ExpressionTree {
    ...
    Node constructTree(String[] postfix) {
        ...
        for (int i = 0; i < postfix.length; i++) {
            ...
            if (!isOperator(postfix[i])) { /*Verifica si es un operando y lo envia a la salidad*/
                t = new Node(postfix[i]);
                st.push(t);
            } else /*Verifica si es un operador y lo envia a la pila*/
            {
                t = new Node(postfix[i]);
